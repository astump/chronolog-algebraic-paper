...
% helpers
\newcommand{\freshTypeMetavariable}[1]{#1 ~\text{fresh}}
...
\begin{figure}[H]
  ...
  \begin{align*}
    \begin{array}{rrcl}
      ...
      A, B
      & ::= &
      % [Note] this can only appear in datatype definitions
      \iota \mid
      Y \mid
      \sMu{D} \mid
      \sFApp{D}{A} \mid
...
\begin{figure}[H]
  ...
  \begin{mathpar}
    ...
    \inferrule[InferCon]{
      ...
      \\\\
      (\forall i)~ A''_i := A'_i[ \iota \mapsto Y ]
      \\\\
      (\forall i)~ \Gamma \vdash a_i : A_i \gsep \mathcal{A}_{a_i} \implies \mathcal{C}_{a_i}
    }{
      \Gamma \vdash D.k(\overline{a}) : \sFApp{D}{Y} \gsep \bigcup_i \mathcal{A}_{a_i} \implies \bigcup_{i} (\mathcal{C}_{a_i} \cup \{ A_i <:: A''_i \})
    }
    ...
    \inferrule[InferGamma]{
      ...
      \Gamma \vdash \gamma a . \{ \overline{k_i(\overline{x_{ij},}) \mapsto b_i ~ |} \} : B \gsep \mathcal{A}_a \cup \bigcup_i \mathcal{A}_{b_i} \implies \mathcal{C}_a \cup \{ A <:: \sMu{D} \} \cup \bigcup_i (\mathcal{C}_{b_i} \cup \{ B_i <:: B \})
    }

    % [Bookmark] This is where the assumption about omega type R is introduced.
    \inferrule[InferOmega]{
      % [TODO] should this be "type variable" and then refer the the substitutable type variables as "type metavariables"?
      R ~\text{is a fresh type constant}
      \\\\
...
\begin{figure}[H]
  \centering
  [TODO] Rules for inferring coercion types, $c : A \sArr B$
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{mathpar}
    ...
    \inferrule[Bool.true]{
      \freshTypeMetavariable{Y}
    }{
      \sTrue : () \sArr \sFApp{\sBool}{Y}
    }

    \inferrule[Bool.false]{
      \freshTypeMetavariable{Y}
    }{
      \sFalse : () \sArr \sFApp{\sBool}{Y}
    }
    ...
    \inferrule[Nat.zero]{
      ...
      \sZero : () \sArr \sFApp{\sNat}{Y}
    }

    \inferrule[Nat.suc]{
      \freshTypeMetavariable{Y}
    }{
      \sSuc : (Y) \sArr \sFApp{\sNat}{Y}
    }
    ...
    \inferrule[NatList.nil]{
      ...
      \sNil : () \sArr \sFApp{\sNatList}{Y}
    }

    \inferrule[NatList.cons]{
      \freshTypeMetavariable{Y}
    }{
      \sCons : (\sNat, Y) \sArr \sFApp{\sNatList}{Y}
    }
  \end{mathpar}
\end{figure}
