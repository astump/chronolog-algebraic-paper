...
%
\author{%
Henry Blanchette\inst{1}\orcidID{0000-0002-9415-0944} \and
Aaron Stump\inst{2}\orcidID{0000-0002-9720-0003}%
}
%
...
\email{aaron.stump@bc.edu}}
%
\maketitle              % typeset the header of the contribution
%
\begin{abstract}
This paper presents a system for more convenient algebraic
programming, where datatypes are defined as least fixed points of covariant
signature functors, and recursive functions are defined as homomorphisms from
initial algebras. This approach requires rolling and unrolling fixed points, and
conversion of algebras to functions that recurse over data. To alleviate the
burden of these extra operations, in this paper we propose to insert them
automatically, using coercive subtyping. The type checker generates subtyping
constraints, which are then solved with the help of a custom logic programming
engine called Chronolog. The subtyping axioms are given to Chronolog as rules,
which uses them to solve the subtyping constraints. To avoid infinite
applications of rules for unrolling signature functors, Chronolog provides a
mechanism for suspending and resuming certain forms of constraints. The
subtyping axioms presented to Chronolog are an equivalent algorithmic version of
the usual declarative rules for algebraic programming.
...
rules.  Chronolog then uses standard SLD resolution
(\cite{apt+82,Kowalski1974}) to search for proofs of the constraints,
instantiating type variables during proof search via unification.  The
rules for rolling and unrolling signatures would, if handled naively,
lead to diverging proof search.  Instead, Chronolog provides a
mechanism to indicate that certain problematic constraints should be
suspended.  These suspended constraints may be resumed if they are
instantiated during solving of other constraints.  If not, an outer
loop of our algorithm handles them specially.
...
expressions $e$. Datatypes are based on signature functors $F$, which
we take to be polynomial functors for simplicity. Least fixed points
of signature functors, referenced by datatype name $D$, are denoted
$\sMu{D}$.  We have usual function types $A \sArr B$.  Types $D \sAlg
B$ are the types for algebras over $\sMu{D}$ computing a value of type
$B$, the carrier of the algebra.  Recursion type variables
$\sRecVar{f}$ are introduced in the bodies of algebras, where $f$ is
the name of the function introduced by the respective algebra.  While
type variables (which are unrelated to recursion type variables)
appear during type checking, as detailed in
...
from $\sRecVar{f}$ (the recursion type variable) to $X$. The second
input is a value $x$ of type $\sFunctorApp{F}{\sRecVar{f}}$.  This is
similar to the input for an $F$-algebra, but with $\sRecVar{f}$ in
place of the carrier.  The recursion type variables are abstract: no
other information about them is available to algebras.

Mendler algebras are closer to usual programming practice, because the
algebra may pattern-match on $x$ to reveal subdata of type
$\sRecVar{f}$, and then apply $f$ to those to recurse.  This is
syntactically similar to making a recursive call in a standard
...
subtrahend \verb|n|.  The code begins with \verb|ω sub'(n)|, which
introduces the name \verb|sub'| for recursive calls, and the input value
\verb|n|.  The code then matches on this \verb|n| with $\gamma$,
...
unrolling \verb|μ Nat| with \verb|unroll id|, written \verb|unroll id - sub' n'|.
The \verb|id| is
...
coerced to \verb|μ Nat| by \verb|roll|, written \verb|roll id - Zero|.

The function \verb|subs| does
...
subtrahend, written \verb|cata id id - sub h1 h2|, where the the coercion operator \verb|-| binds more tightly than application (i.e. the expression is parsed equivalently as \verb|(cata id id - sub) h1 h2|).
Note that \verb|subs| returns a list as long as the shorter of its inputs,
similar to \verb|zip| in Haskell.
...
terminate on all inputs.  Nonterminating recursive calls are
disallowed just by typing.  Nevertheless, we expect the reader will
agree that the code is rather burdened with coercions.  This raises
the question we address in this paper: can we support algebraic
...
algorithmic when read from conclusions to premises: they cannot be
used directly as a logic programd. This is due to the \IRule{Trans}
rule, which introduces a new metavariable in its premises that does
not appear in in its conclusion, and so can be applied ad infinitum.
So we next propose an algorithmic version, where all metavariables in
the premises of the rule are already present in the conclusion, and
the sizes of the constraints decrease.  This ensures that SLD
resolution will not diverge on ground constraints.  We prove that the
two sets of rules are equivalent.  Then we can formulate the coercion
typing rules.
...
the definition of subtyping as a preorder.

% [HENRY] This discussion was shortened and moved to beginning of section 5, as
%         per reviewer A's suggestion
%
% While these rules are intuitive and express the coercions needed, they are not
% algorithmic when read from conclusion to premises.  The reason, as mentioned
% in the introduction, is the \IRule{Trans} rule, which introduces a new
% metavariable in the premises.  Proof search would diverge with this rule.  We
...
\begin{lemma}[preorder algorithmic]
  \
...
Most of the coercion typing rules have uniquely associated coercions.
For example, \IRule{ArrCoe} is associated with the coercion \verb|arr| and \IRule{CovCoe} is associated with the coercion \verb|cov|.
However, the base case coercion typing rules \IRule{ReflDataCoe} and \IRule{RecVarCoe} are each associated merely with the identity coercion \verb|id| (at their respective types).
These account for why the examples in Figure~\ref{fig:example-subs-algebraic-with-coercions} require uses of \verb|id| at the leaves of each coercion.
While \lcata does not support universal quantification in types, these coercions are polymorphic primitives, where the type arguments are always inferred during type checking.
...
We omit a rigorous formalization of the coercion elaboration phase (the algorithmic insertion of coercions is fully realized in our implementation of type checking), however in order to reinforce that generated constraints correspond to specific sub-terms, we highlight the sub-term and the linked generated constraint with the same color in the rules of Figure~\ref{fig:constraint-generation-rules}.
...
A derivation of $\Gamma \vdash a : A \mid \constraints$ justifies that $a$ has type $A$ \textit{only if} the constraint problem $\constraints$ is solvable.
...
\begin{lemma}[Soundness of \Func{Generate}]
  \label{lem:generate-soundness}
  If $\sigma$ is a solution to the constraint problem $\constraints = \Func{Generate}(\Gamma, a, A)$, then $\Gamma \vdash_{\lcata} a : A$
\end{lemma}
\begin{proof}
  Suppose that $\sigma$ is a solution to the constraint problem $\constraints = \Func{Generate}(\Gamma, a, A)$
  Then $\sigma$ specifies a derivation for each constraint $A \subtypeA B$ in $\constraints$.
  Use these derivations along with the rule \IRule{Subtype} to construct a derivation of $\Gamma \vdash_{\lcataSub} a : A$.
  Then, replace each use of \IRule{Subtype} in this typing derivation with the corresponding derivation of \IRule{Coerce}, as described in \ref{sec:typing-coercions}.
\end{proof}
...
We take a logic-programming approach to solving problems $\constraints$, where the coercions that justify the generated subtyping relations are
...
Whenever a constraint that satisfies $S$ appears, that constraint is immediately suspended.
...
Search branches that reach states where \textit{no} rules may be applied are pruned with no backtracking.
For type checking, we only need to check whether \Func{Solve} results solutions or not.
...
\Func{Refine} is defined by the following cases.
When multiple cases of \Func{Refine} are matched, any case can be chosen arbitrarily.
Why this arbitrary choice preserves completeness is explained in the proof of Lemma~\ref{lem:solver-completeness}.

\begin{mathpar}
  \Func{Refine}(\constraints) := \begin{cases}
    \{ X \mapsto R \} ~\text{if}~ (X \subtypeA R) \in \constraints ~\text{or}~ (R \subtypeA X) \in \constraints
    \\
    \{ X \mapsto \sMu{D} \} ~\text{if}~ (X \subtypeA \sMu{D}) \in \constraints ~\text{or}~ (\sMu{D} \in X) \in \constraints
    \\
...
\begin{lemma}[Decrease of \Func{Refine}]
...
\end{lemma}
\begin{proof}
  Proof of termination proceeds by lexicographic induction on $(s, v)$, where $s$ is the size of $\constraints$ and $v$ is the number of unique variables in $\constraints$.
  Each iteration of the \textbf{while}-loop in \Func{Solve} begins with an invocation of $\Func{Chronolog}(S, \constraints)$.
  If \Func{Chronolog} results in $\langle \text{``failure''} \rangle$ or $\langle \text{``success''} \rangle$, then we are done.
  Otherwise, \Func{Chronolog} results in $\langle \text{``progress''}, \sigma', \constraints' \rangle$, in which case the new value assigned to $\constraints$ is $(\Func{Refine}(\constraints') \circ \sigma') (\constraints')$.
  Let $(s', v')$ be the size and number of unique variables respectively in $(\Func{Refine}(\constraints') \circ \sigma') (\constraints')$.
  There are two cases:
  \begin{enumerate}
  \item
    If $\Func{Chronolog}(S, \constraints)$ used \textit{no} rule applications, then it must have merely suspended all goals, so $\constraints' = \constraints$, $\sigma = \text{id}$, and $s' = s$.
    The constraints $(\Func{Refine}(\constraints') \circ \sigma') (\constraints') = (\Func{Refine}(\constraints')) (\constraints')$ has fewer unique variables than $\constraints$ by Lemma~\ref{lem:refine-decreases}, so $v' < v$ and thus $(s', v') = (s, v') < (s, v)$.
  \item
    If $\Func{Chronolog}(S, \constraints)$ used \textit{some} rule applications, then $s' < s$ by Lemma~\ref{lem:decrease}, and so $(s', v') < (s, v)$.
  \end{enumerate}
\end{proof}
...
\begin{proof}
  ...
  \Func{Refine}'s handling of $X \subtypeA \sMu{D}$ and $\sMu{D} \subtypeA X$ constraints does not violate completeness since, any other appearances of $X$, once substituted for $\sMu{D}$, may be rolled or unrolled by subsequent invocations of \Func{Chronolog} to satisfy the constraints at those appearances.
  \Func{Refine}'s handling of $X \subtypeA \sFunctorApp{D}{A}$ and $\sFunctorApp{D}{A} \subtypeA X$ constraints does not violate completeness since, as there is an assumed solution to $\constraints$, $A$ must be of the form $\sFunctorApp{D}{(\dots (\sFunctorApp{D}{Y}))}$ (where $Y$ is a distinct type variable from $X$) or $\sFunctorApp{D}{(\dots (\sMu{D}))}$, and so any other appearances of $X$ may be rolled or unrolled by subsequent invocations of \Func{Chronolog} to satisfy any other relations to the constraints at those appearances.
...
\begin{theorem}[Soundness of \Func{TypeCheck}]
  \label{thm:typecheck-soundness}
  If $\Func{TypeCheck}(\Gamma, a, A) = \top$ (Algorithm~\ref{alg:type-checker}) then $\Gamma \vdash_{\lcataSub} a : A$
\end{theorem}
\begin{proof}
  Suppose $\Func{TypeCheck}(\Gamma, a, A) = \top$.
  Then there exists a solution to the set of constraints $\Func{Generate}(\Gamma, a, A)$, which are sufficient to demonstrate $\Gamma \vdash_{\lcata} a : A$ (by Lemma~\ref{lem:generate-soundness}).
\end{proof}

Finally, formalize the relationship between $\lcata$ and $\lcataSub$ in Theorem~\ref{thm:equivalence-lcata} in terms of erasure of explicit coercions $\lcata$ to implicit coercions in $\lcataSub$ that are inferred during type checking.

\begin{definition}[Erasure of terms from $\lcata$ to $\lcataSub$]
  Let $a$ be a term of $\lcata$. Then $|a|$ is the \textit{erasure} of $a$ into $\lcataSub$, which is defined by replacing in $a$ each appearance of $\sCoerce{c}{a'}$ with $a'$.
\end{definition}

\begin{theorem}[Equivalence of $\lcata$ and $\lcataSub$]
  \label{thm:equivalence-lcata}
  \begin{enumerate}
    \item $\Gamma \vdash_{\lcata} a : A \implies \Gamma \vdash_{\lcataSub} |a| : A$
    \item $\Gamma \vdash_{\lcataSub} a : A \implies \exists a' . |a'| = a ~\text{and}~ \Gamma \vdash_{\lcata} a' : A$
  \end{enumerate}
\end{theorem}
...
Now, with the machinery of coercive subtyping in place, we can write those examples without any coercions, as demonstrated in Figure~\ref{fig:example-subs-algebraic-without-coercions}.
...
subs : NatList ⇒ μ NatList → μ NatList =
  ...
  \end{verbatim}
  \caption{Code for \texttt{sub} and \texttt{subs} with implicit coercions.}
...
algebras and final coalgebras~\cite{charity92}.  Recursion is done
...
ideas~\cite{tofu10}.  Charity and Tofu both support coinductive
datatypes, which we did not study here.

Folding and unfolding signature functors is reminiscent of folding and
unfolding recursive types as presented, for example, by
Pierce~\cite{pierce02}.  The situation here is much simpler, though,
because we consider the signature functors nominally.  That is,
recursive types $\sMu{D}$ and $\sMu{D'}$ are never related in our
theory, even if names $D$ and $D'$ are associated with the same
underlying signature functor.  In contrast, equirecursive types are
treated extensionally, and this requires a greatest fixed-point
algorithm to test equivalence.  It is not our purpose to handle this
more general situation.  Rather, the goal is to handle standard
inductive datatypes, which are usually treated nominally in functional
programming.  For example, two copies of the list datatype are not
considered interchangeable in languages like Haskell or OCaml.
...
considered by~\cite{abreu+23}.  There, algebras call functions to
...
\textbf{Acknowledgments.} Thanks to the anonymous reviews for very helpful
detailed feedback, which has greatly improved the paper.
