...
\begin{figure}[htb]
  ...
  \begin{align*}
    \begin{array}{lrcl}
      ...
      F &::=& \lambda X . \sepBy{k(\sepBy{P(X)}{~\times})}{~+}
      \\[0.5em]
      %
...
% The type system for \lcata is basically the same as the simply-typed $\lambda$-calculus, but with a few extra constructs.

% % \typingRules

% \lcata supports exhaustive pattern-matching with the $\gamma a . \{ \dots \}$ construct.
% The typing rule for this construct is \IRule{Gamma}.
% The typing rule \IRule{Gamma} looks up the signature functor of the datatype of the term that is being discriminated by the pattern-match.
% As a functor, the signature functor is parametrized by a single type parameter, which \IRule{Gamma} instantiates with the type argument $C$ corresponding to deriving $\Gamma \vdash a : D ~ C$.

% \lcata has a special construct for introducing algebras, $\omega f(x) . b$.
% The typing rule for this construct, \IRule[Omega], must take into account that a new type, $\sRecVar{f}$,  is introduced into the local context of $b$, where $\sRecVar{f}$ is associated specifically with this syntactic construct, the fact of which is encoded by the $f$ index of $\sRecVar{f}$.
% \lcata does not support type inference or polymorphism, so this is just a very restricted form of scoped typing for the sake of modeling Mendler algebras.
% Such a recursion type variable $\sRecVar{f}$ is not related to any other types, which ensures in a similar way that parametricity does for Mendler algebras, that $f$ can only ever be called on sub-data of $x$. 

The type system of \lcata conservatively extends that of the simply-typed $\lambda$-calculus with specialized constructs for pattern matching and algebraic recursion.
A sampling of the notable typing rules of \lcata are presented in Figure~\ref{fig:typing-rules}, and the full set of typing rules are provided in Appendix~\ref{app:typing-rules}
...
Because the signature functor abstracts over a single type parameter, \IRule{Gamma} instantiates it with the type argument $C$, satisfying the typing premise $\Gamma \vdash a : \sFunctorApp{D}{C}$.
...
\begin{lemma}[Completeness of \Func{Solve}]
  ...
  If $\sigma$ is a solution to a constraint problem $\constraints$, then $\eqClass{\sigma}{\sim} \cap \Func{Solve}(\constraints) \neq \varnothing$.
\end{lemma}
\begin{proof}
  \Func{Solve} finds all solutions to $\constraints$ up to $\sim$ because \Func{Chronolog} attempts every valid application of the algorithmic subtyping rules which are complete by Theorem~\ref{thm:completeness}, \Func{Refine} preserves validity of solution $\sigma$, and \Func{Solve} terminates by Theorem \ref{lem:solver-termination}.
\end{proof}
