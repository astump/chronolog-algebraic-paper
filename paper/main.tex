...
\begin{algorithm}[htb]
  ...
  \begin{algorithmic}[1]
    ...
    \State \qquad $\mathit{constraint} \gets \Func{Sample}(\constraints')$
...
\begin{lemma}[Termination]
  ...
  For any constraint problem $\constraints$, $\Func{Solve}(\constraints)$ terminates.
\end{lemma}
\begin{proof}
...
\begin{lemma}[Completeness]
  ...
  If $\sigma$ is a solution to a constraint problem $\constraints$, then $\exists \sigma' \in \eqClass{\sigma}{\sim}$ such that $\sigma' \in \Func{Solve}(\constraints)$.
\end{lemma}
\begin{proof}
