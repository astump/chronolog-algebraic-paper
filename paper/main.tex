...
\begin{figure}[H]
  ...
  \begin{align*}
    \begin{array}{rrcl}
      ...
      \lambda (x : A) . b \mid
      (f ~ a) \mid
      \omega(D, B, R, f, x) . b \mid
      \\[0.5em] &&&
      \gamma(D, B) a . \{ \overline{k(\overline{x}) \mapsto b ~ |} \} \mid
...
\begin{figure}[H]
  ...
  \begin{mathpar}
    ...
    \inferrule[InferGamma]{
      ...
      \Gamma \vdash \gamma(D, B) a . \{ \overline{k_i(\overline{x_{ij},}) \mapsto b_i ~ |} \} \gsep \mathcal{A}_a \cup \bigcup_i \mathcal{A}_{b_i} \implies \mathcal{C}_a \cup \{ A <:: \mu D \} \cup \bigcup_i (\mathcal{C}_{b_i} \cup \{ B_i <:: B \})
    }

    % [Bookmark] This is where the assumption about omega type R is introduced.
    \inferrule[InferOmega]{
      \Gamma, (R : \ast), (f : R \sArr A), (x : D ~ R) \vdash b : B \gsep \mathcal{A}_b \implies \mathcal{C}_b
    }{
