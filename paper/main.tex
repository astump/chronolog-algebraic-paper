...
\begin{figure}[H]
  ...
  \begin{mathpar}
    ...
    \inferrule[InferVar]{
      ...
      \Gamma \vdash x : A \mid \varnothing
    }
    ...
    \inferrule[InferCon]{
      ...
      \\\\
      (\forall i)~ \Gamma \vdash a_i : A_i \mid \mathcal{C}_{a_i}
    }{
      \Gamma \vdash D.c(\dots a_i, \dots) \mid \bigcup_{i} (\mathcal{C}_{a_i} \cup \{ A_i <: \textsf{freshenFunctorParam}(A'_i) \})
    }

    \inferrule[InferLam]{
      \Gamma, (x : A) \vdash b : B \mid \mathcal{C}_b
    }{
      \Gamma \vdash \lambda (x : A) . b : A \sArr B \mid \mathcal{C}_b
    }

    \inferrule[InferApp]{
      A', B ~\text{are fresh type variables}
      \\\\
      \Gamma \vdash f : F \mid \mathcal{C}_f \quad
      \Gamma \vdash a : A \mid \mathcal{C}_a
    }{
      \Gamma \vdash (f ~ a) : B \mid \mathcal{C}_f \{ F <: A' \sArr B \} \cup \mathcal{C}_a \cup \{ A <: A' \}
    }

    \inferrule[InferGamma]{
      \langle D , V \rangle \in \mathscr{D} \\\\
      (\forall i)~ \langle c_i , \langle \dots A_{ij}, \dots \rangle \rangle \in V
      \\\\
      (\forall i)~ \Gamma, \dots x_{ij} : A_{ij} \dots \vdash b_i : B_i \mid \mathcal{C}_{b_i}
    }{
      \Gamma \vdash \gamma(D, B) a . \{ \dots ~ c_i(\dots x_{ij}, \dots) \mapsto b_i | ~ \dots \} \mid \bigcup_i (\mathcal{C}_{b_i} \cup \{ B_i <: B \})
    }

    \inferrule[InferOmega]{
      \Gamma, (R : \ast), (f : R \sArr A), (x : D ~ R) \vdash b : B \mid \mathcal{C}_b
    }{
      \Gamma \vdash \omega(D, B, R, f, x) . b : D \sAlg B \mid \mathcal{C}_b
    }

    \inferrule[InferLet]{
      \Gamma \vdash a : A \mid \mathcal{C}_a \quad
      \Gamma, (x : A') \vdash b : B \mid \mathcal{C}_b
    }{
      \Gamma \vdash \sLet x : A' = a \sIn b : B \mid \mathcal{C}_a \cup \{ A <: A' \} \cup \mathcal{C}_a
    }
    ...
    \inferrule[InferCoerce]{
      ...
      \Gamma \vdash \sCoerce c ~ a : B \mid \mathcal{C}_a \cup \{ A <: A' \}
    }
