...
\begin{figure}[H]
  ...
  \begin{align*}
    \begin{array}{rrcl}
      % [Note from Henry]: actually we don't want this in the syntax since these are only metavariables, not part of syntax
      % %
      % \text{(Type variables)} &
      % Y &\in& \mathscr{Y}
      % \\[1em]
...
\begin{figure}[H]
  ...
  \begin{mathpar}
    ...
    % [TODO]: The current way this is set up, it allows you to always have the option to put the coercion on the argument, or by using the covariance coercion to put it on the domain of the function. I don't think it's good to always have this option, so one way to resolve this would be to only allow putting the co-version on the codomain of a function and never on the domain. I don't think it's good to always have this option, so one way to resolve this would be to only allow putting the co-version on the codomain of a function and never on the domain. Then at Applications, you can always allow wrapping a coercion around the argument.
    \inferrule[InferApp]{
    ...
    \inferrule[InferLet]{
      ...
      \Gamma \vdash \sLet x = a \sIn b : B \gsep \mathcal{A}_a \cup \mathcal{A}_b \implies \mathcal{C}_a \cup \mathcal{C}_a
    }
...
\begin{figure}[H]
  ...
  \begin{mathpar}
    ...
    \inferrule[ReflDataCoe]{
      ...
      \Gamma \vdash \sIdData ~ D : \sCoerceSig{\sMu{D}}{\sMu{D}}
    }
    ...
    \inferrule[CovCoe]{
      ...
      \Gamma \vdash \sCov : \sCoerceSig{\sFunctorApp{D}{A}}{\sFunctorApp{D}{A'}}
    }

    \inferrule[AlgCoe]{
      \Gamma \vdash c : \sCoerceSig{A}{A'}
    }{
      \Gamma \vdash \sSubAlg ~ D \sCoerceSig{D \sAlg A}{D \sAlg A'}
    }
    ...
    \inferrule[Roll1Coe]{
      ...
      \Gamma \vdash \sRoll ~ D ~ c \vdash \sCoerceSig{\sFunctorApp{D}{A}}{\sMu{D}}
    }
    ...
    \inferrule[Roll2Coe]{
      ...
      \Gamma \vdash \sUnroll ~ D ~ c \vdash \sCoerceSig{\sFunctorApp{D}{A}}{\sMu{D}}
    }
  ...
  \caption{Coercion type inference rules.}
  \label{fig:coercion-type-inference-rules}
\end{figure}
