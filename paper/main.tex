...
\begin{definition}[Erasure of terms from $\lcata$ to $\lcataSub$]
  Let $a$ be a term of $\lcata$. Then $|a|$ is the \textit{erasure} of $a$ into $\lcataSub$, which is defined by structurally recursing on $a$ to replace each appearance of $\sCoerce{c}{a'}$ with $a'$.
\end{definition}

\begin{theorem}[Equivalence of $\lcata$ and $\lcataSub$]
  $\Gamma \vdash_{\lcata} a : A \iff \Gamma \vdash_{\lcataSub} |a| : A$.
\end{theorem}
