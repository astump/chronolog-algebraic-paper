...
The type system of \lcata conservatively extends that of the simply-typed $\lambda$-calculus with specialized constructs for pattern matching, algebraic recursion, and explicit coercion.
...
\begin{figure}[htb]
  ...
  \caption{Notable typing rules for \lcata}
...
This rule relies upon a dedicated coercion typing judgment of the form $\Gamma \vdash c : A \subtypeA B$, which is formalized by the inference rules presented in Figure~\ref{fig:coercion-typing-rules}.
However, prior to detailing the coercion typing semantics, it is necessary to establish the underlying subtyping relation, which we develop in the subsequent section.
