...
introduced by the respective algebra.  While type variables (which are unrelated
to recursion type variables) appear during type checking, as detailed in
Section~\ref{sec:typechecking}, type variables cannot appear in the surface
syntax for types.  In this way, all types that appear in programs are ground
types.
...
Crucially, while type variables are introduced during the type-checking phase, they serve exclusively to facilitate constraint satisfaction rather than to support type inference. 
Both $\lambda$-abstractions and $\textsyntax{let}$-bindings require explicit type annotations, thereby ensuring that all types occurring within a program are ground. We defer the development of a comprehensive type inference mechanism to future work.

Initially, the constraint generation algorithm, denoted $\Func{Generate}(\Gamma, a, A)$, produces a set of subtyping constraints for a term $a$ against an expected type $A$ under the context $\Gamma$.
This procedure operates by structural recursion on $a$, governed by the rules delineated in Figure~\ref{fig:constraint-generation-rules}.
Each emitted constraint signifies an implicit application of the \IRule{Subtype} rule, which is requisite for constructing a valid typing derivation.
Furthermore, every constraint is explicitly annotated with the precise sub-term at which the subsumption step occurs.
Under the paradigm of coercive subtyping, these applications of \IRule{Subtype} are witnessed by coercions that provide computational justification for the subtyping relation.
Operationally, these coercions are reified as explicit function applications which are inserted at the annotated sub-terms associated with the resolved constraints.
...
\todoHenry{Then, these subtyping constraints are solved by a logic-programming engine by applying the algorithmic subtyping rules in Figure~\ref{fig:algorithmic-subtyping-rules}, where each rule corresponds to a constructor of the coercion to be inserted at the constraint's associated position in the term.
}

\todoHenry{Some of these coercions may end up being the identity function (if the justified  subtyping relation is of the form $A <:: A$), or semantically equivalent to the identity (in the case of \textsyntax{roll} and \textsyntax{unroll}, which are used only for typing but are semantically interpreted as teh identity).
So, there are many syntactically different ways to solve a set of subtyping constraints.
We define an equivalence relation $\sim$ over solutions where two solutions are related if they result in semantically equivalent terms when applied to the term that generated the constraints}
