...
algorithmic when read from conclusions to premises due to the presence of the
\IRule{Trans} rule, which introduces a new metavariable in its premises that
does not appear in in its conclusion, and so can be applied ad infinitum.  So we
next propose an algorithmic version, where all metavariables in the premises of
the rule are already present in the conclusion, and the sizes of the constraints
decrease.  This ensures that selective definite linear clause (SLD)
resolution~\cite{Kowalski1974} will not diverge on ground constraints.  We prove
that the two sets of rules are equivalent.  Then we can formulate the coercion
typing rules.
...
For example, if we have constraints $\{ X <:: R, X <:: \sMu{D} \}$, then we \textit{must} choose refine by $X \mapsto R$ since the other choice of $X \mapsto \sMu{D}$ (refined to $\sMu{D} <:: R$) would leave the constraint $X <:: R$ unsolvable.
