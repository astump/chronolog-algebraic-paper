...
\begin{mathpar}
  \begin{array}{rlrll}
    ...
    No && \Func{Refine}(\{ \dots, X &\subtypeA Y, \dots \}) &:= \{ X \mapsto Y \} \\
...
\end{lemma}
\begin{proof}
  We show termination by demonstrating that \todoHenry{define what the termination metric is} is a strictly decreasing metric over each iteration of the while loop in \Func{Solve}.
  ...
  The substitution resulting from $\Func{Refine}(\constraints)$ preserves the size of constraints in all but one case, $\Func{Refine}(\{ \dots, X \subtypeA \sFunctorApp{D}{A}, \dots \}$ or $\Func{Refine}(\{ \dots, \sFunctorApp{D}{A} \subtypeA X, \dots \}$, where it results in $\{ X \mapsto \sFunctorApp{D}{Y} \}$ where $Y$ is a fresh type variable.
  In this case, \todoHenry{argue that even in this case, decreasing metric}
\end{proof}
