...
subtyping rules.  We have reflexivity rule \textsc{Refl} for datatypes.  \textsc{Arr} is the usual
...
is no rule for transitivity.  In effect, transitivity needs to be
built into all the other rules.  We see a similar change from the
declarative to the algorithmic versions of the \textsc{Roll} and
\textsc{Unroll} rules, for rolling and unrolling the signature
functor.
...
{X : Tp} → D⇒ X <: μD ⟶ X
\end{verbatim}
\noindent This is a dependent function type, stating that give \verb|X : Tp|,
the \verb|Cata| constructor will return a proof of \verb|D⇒ X <: μD ⟶ X|,
expressing the coercion from an algebra with carrier \verb|X| to its catamorphism.
