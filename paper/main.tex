...
\begin{theorem}[Completeness of \Func{TypeCheck}]
  ...
  If $\Gamma \vdash a : A$, then $\Func{TypeCheck}(\Gamma, a, A) = \top$ (Algorithm~\ref{alg:type-checker}).
\end{theorem}
\begin{proof}
  Suppose $\Gamma \vdash a : A$.
  $\Func{Generate}(\Gamma, a, A)$ terminates (by induction over $a$) and generates a set of constraints, $\constraints$, which are sufficient to demonstrate $\Gamma \vdash a : A$.
