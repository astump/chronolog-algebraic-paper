...
\todoHenry{call back to where i was talking about this before, where I defined $\sim$ for constraint solutions}
...
\end{lemma}
\begin{proof}
  We show termination by demonstrating that the number of type variables among constraints is a strictly decreasing metric over each iteration of the while loop in \Func{Solve}.
  ...
  The substitution resulting from $\Func{Refine}(\textit{constraint})$ preserves the size of constraints in all but one case, $X \subtypeA \sFunctorApp{D}{Y}$ or $\sFunctorApp{D}{Y} \subtypeA X$, where it results in $\{ X \mapsto \sFunctorApp{D}{Y} \}$.
  In this case, 
\end{proof}
