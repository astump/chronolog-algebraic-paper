...
algorithmic when read from conclusions to premises due to the presence
of the \IRule{Trans} rule, which introduces a new metavariable in its
premises that does not appear in in its conclusion, and so can be
applied ad infinitum.  So we next propose an algorithmic version,
where all metavariables in the premises of the rule are already
present in the conclusion, and the sizes of the constraints decrease.
This ensures that SLD resolution will not diverge on ground constraints.
We prove that the two sets of rules are equivalent.  Then we can
formulate the coercion typing rules.
